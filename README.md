...
Les requêtes V3 utilisent le nouveau format des fichiers de l'INSEE et sont donc divisées en deux ressources distinctes : les unités légales (SIREN) et les établissements physiques (SIRET).

## Endpoints V3

Unités légales :

    curl 'localhost:3000/v3/unites_legales/:SIREN' # recherche par SIREN
    curl 'localhost:3000/v3/unites_legales/' # Index

Établissements :

    curl 'localhost:3000/v3/etablissements/:SIRET' # recherche par SIRET
    curl 'localhost:3000/v3/etablissements/' # Index
...
Pour ces endpoints, vous pouvez filtrer les résultats par n'importe lequel des champs des ressources demandées en passant les paramètres dans la requête. Exemple :
    ...
    # Renvoie tout les établissements actifs correspondants au siren 345184428
...
Les informations relatives aux stocks sont enregistrées en base de donnée. Un stock avec un statut PENDING ou LOADING indique une opération en cours et empêche de relancer l'opération d'import.
